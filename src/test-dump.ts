describe('IntlMessageFormat', function () {
  ...
  describe('arguments with', function () {
    describe('no spaces', function () {
      ...
      it('should fail when the argument in the pattern is not provided', function () {
        expect(msg.format).toThrow(Error('The intl string context variable "STATE" was not provided to the string "{STATE}"'));
      });
      ...
      it('should fail when the argument in the pattern has a typo', function () {
        ...
        expect(formatWithValueNameTypo).toThrow(
          Error('The intl string context variable "STATE" was not provided to the string "{STATE}"'),
        );
      });
    ...
    describe('a numeral', function () {
      ...
      it('should fail when the argument in the pattern is not provided', function () {
        ...
        expect(formatWithMissingValue).toThrow(
          Error('The intl string context variable "ST1ATE" was not provided to the string "{ST1ATE}"'),
        );
      });
  ...
  describe('exceptions', function () {
    ...
    it('should take negative number as plural', function () {
      const msg = '{num, plural, offset:-1 =-1{negative one} one{one} other{other}}';
      const mf = new IntlMessageFormat(msg, 'en');
  ...
  describe('xml', function () {
    ...
    it('should handle tag in plural', function () {
      const mf = new IntlMessageFormat('You have {count, plural, =1 {<b>1</b> Message} other {<b>#</b> Messages}}', 'en');
      expect(
        mf.format<string>({
          b: (chunks) => `{}${chunks}{}`,
          count: 1000,
  ...
  describe('# symbol in plural rule argument', () => {
    ...
    it('does not format # symbol in the plural option as a standalone part', () => {
      const mf = new IntlMessageFormat('You {count, plural, one {worked for # hour} other {worked for # hours}} today.', 'en');
      expect(mf.formatToParts({ count: 1 })).toEqual([{ type: PART_TYPE.literal, value: 'You worked for 1 hour today.' }]);
    });
    ...
    it('does not replace # symbol in deeply nested sub messages', () => {
      ...
      expect(mf.format({ count: 1, unit: 'hour' })).toBe('You worked for # hour today.');
      expect(mf.format({ count: 3, unit: 'hour' })).toBe('You worked for # hours today.');
    });
  });
  ...
  it('number skeleton', function () {
    expect(
      new IntlMessageFormat('{amount, number, ::currency/CAD .0 group-off}', 'en-US').format({
        amount: 123456.78,
      }),
    ).toMatch(/\$123456.8/); // Deal w/ IE11
    expect(
      new IntlMessageFormat('{amount, number, ::currency/GBP .0#}', 'en-US').format({
        amount: 123456.789,
      }),
    ).toBe('£123,456.79');
  });
  ...
  it('number skeleton with scale', function () {
    expect(
      new IntlMessageFormat('{amount, number, ::percent scale/0.01}', 'en-US').format({
        amount: 12.3,
      }),
    ).toBe('12%');
  });
